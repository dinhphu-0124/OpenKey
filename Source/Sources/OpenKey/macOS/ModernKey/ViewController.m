...
- (void)viewDidAppear {
  ...
  self.view.window.title = [NSString
      stringWithFormat:
          @"OpenKey %@ [Debug UI Preview] - Bộ gõ Tiếng Việt",
          [[NSBundle mainBundle]
              objectForInfoDictionaryKey:@"CFBundleShortVersionString"]];
#else
...
- (IBAction)onFanpageLink:(id)sender {
  [[NSWorkspace sharedWorkspace]
      openURL:[NSURL URLWithString:@"https://github.com/dinhphu-0124/OpenKey"]];
}
...
- (IBAction)onCheckNewVersionButton:(id)sender {
  ...
  [OpenKeyManager checkNewVersion:self.view.window
                     callbackFunc:^{
